  ...
  useEffect(() => {
    ...
    async function loadProjectDefaults() {
      try {
        ...
        setCompanyName((prev) => (prev.trim().length > 0 ? prev : project.name || ''));
        setDescription((prev) => (prev.trim().length > 0 ? prev : project.description || ''));

        const contact = project.contact;
        if (contact) {
          setPhone((prev) => (prev.trim().length > 0 ? prev : contact.phone || ''));
          setEmail((prev) => (prev.trim().length > 0 ? prev : contact.email || ''));
          setAddress((prev) => {
            if (prev.trim().length > 0) return prev;
            if (contact.address && contact.address.trim()) return contact.address;
            const cityLine = [contact.district, contact.city].filter(Boolean).join(' / ');
            return cityLine || '';
          });
        }
      } catch {
