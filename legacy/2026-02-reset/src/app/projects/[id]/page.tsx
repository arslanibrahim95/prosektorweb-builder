...
export default function ProjectDetailPage() {
  ...
  const handlePublish = async () => {
    try {
      ...
      setPublishMessage(
        `Yayinlama tamamlandi (${result.pagesPublished ?? 0} sayfa). ${qualityNote} ${webhookNote}`
      );
    } catch (publishError) {
  ...
  const cityLine = [project.contact?.district, project.contact?.city].filter(Boolean).join(' / ') || null;
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-white">{getOsgbTemplateLabel(project.template)}</p>
                  </div>
                  <div>
                    <label className="text-sm text-slate-400">Sektör</label>
                    <p className="font-medium text-white">{OSGB_INDUSTRY}</p>
                  </div>
                  <div>
                    <label className="text-sm text-slate-400">Telefon</label>
                    <p className="font-medium text-white">{project.contact?.phone || '—'}</p>
                  </div>
                  <div>
                    <label className="text-sm text-slate-400">Şehir / İlçe</label>
                    <p className="font-medium text-white">{cityLine || '—'}</p>
                  </div>
                  <div className="md:col-span-2">
                    <label className="text-sm text-slate-400">E-posta</label>
                    <p className="font-medium text-white">{project.contact?.email || '—'}</p>
                  </div>
                  <div className="md:col-span-2">
                    <label className="text-sm text-slate-400">Adres</label>
                    <p className="font-medium text-white">{project.contact?.address || '—'}</p>
                  </div>
                </div>
