...
import {
  ...
  getDefaultServerIp,
} from "@/server/integrations/cloudflare";
 ...
 */
async function triggerDockerBuild(
  projectId: string,
 ...
 */
async function triggerK8sDeploy(
  projectId: string,
  domain: string,
 ...
 */
async function directK8sDeploy(
  projectId: string,
  domain: string,
): Promise<K8sDeployResult> {
  ...
  try {
    ...
    const deployResponse = await fetch(
      `${K8S_CLUSTER_URL}/apis/apps/v1/namespaces/${namespace}/deployments`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${K8S_TOKEN}`,
...
export async function exportSite(projectId: string): Promise<ExportResult> {
  try {
    ...
    // 4. Dosyaları yaz
    const filePromises = result.pages.map(async (page) => {
...
export async function createPreview(projectId: string): Promise<DeployResult> {
  try {
    ...
    if (cf && serverIp) {
      ...
      if (result.success && result.fqdn) {
        previewUrl = result.fqdn;
        await prisma.webProject.update({
          where: { id: projectId },
  ...
  try {
    ...
    if (cf) {
      ...
      if (serverIp) {
        ...
        if (zone) {
          await cf.createStandardWebsiteDns(
            zone.id,
            project.domain.name,
            serverIp,
          );
    ...
    // 5. Domain cutover workflow (DNS propagation + SSL + live switch)
    const cutoverResult = await startCutover(project.domain.id);
    if (!cutoverResult.ready) {
      return {
        success: false,
        error: `Domain cutover beklemede: ${cutoverResult.message}`,
      };
    }

    const canonicalUrl = cutoverResult.status.canonicalUrl || `https://${project.domain.name}`;
