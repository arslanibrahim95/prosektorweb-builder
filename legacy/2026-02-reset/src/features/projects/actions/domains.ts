...
export async function getDomainStats() {
    try {
        ...
        const [total, active, pending, suspended, expired] = await Promise.all([
            prisma.domain.count(),
            prisma.domain.count({ where: { status: { in: ['ACTIVE', 'LIVE'] } } }),
            prisma.domain.count({ where: { status: { in: ['PENDING', 'PURCHASED', 'ZONE_CREATED', 'DNS_CONFIGURED', 'DNS_PROPAGATED', 'SSL_PENDING', 'SSL_ACTIVE', 'CUTOVER_READY'] } } }),
